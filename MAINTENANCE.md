...
| 2.3 | 1.4 | 2026-06-12 | Spoke-page topical tightness (operator caught cluster leak on the HCG rewrite: medication-menu table belonged to the hub). New: sibling-service-leak scope test with the three-rule sibling-reference policy (inclusions fine / compatibility FAQs fine / comparisons → hub), `CLUSTER_LEAK` issue_type (IMPORTANT, G2), `LINK_TO_SIBLING` recommended_home, optional `audit_metadata.site_context` operator input (hub + sibling spokes), spoke scope guard mandatory in rewrite_brief.do_not_do (instruction 23), Worked Example 11. Also instruction 24: licensed-clinicians-only rule for regulated-mode team sections (supervising physician leads with board + licence; care-delivering NP/PA/RN included with credential and role; non-clinical staff omitted; person-branded product names exempt) — emitted as a do_not_do guard when the source page mixes clinical and non-clinical personnel. Audit remains page-scoped by default; site context makes cluster boundaries enforceable |
